...
> SCOPE NOTE (ADR-0014): borromeanRings's real research enhancement does NOT search by
> itself — it *steers the wrapped agent's own search* (see the `borromeanrings-research`
...
> visible event trail) with borromeanRings doing the searching — useful for testing the
> ideas, not how borromeanRings enhances research in production.
...
memory) — borromeanRings's anti-hallucination differentiator. Search and fetch are
...
# Performed by the wrapped agent / an LLM (borromeanRings enforces + structures it); injected
...
def verify_claim_adversarial(
    ...
    *same* candidate passage. No qualifying passage ⇒ rejected. More judges that
    must agree = stricter. The judges are the semantic step (agent/LLM); borromeanRings
...
# wrapped agent/LLM (DMQR / pseudo-answer); borromeanRings structures the result.
...
def mutate_queries(query: str, mutator: QueryMutator, max_variants: int = 5) -> list[str]:
    ...
    diverse variants from the mutator (the agent/LLM). borromeanRings enforces: original
...
# (a weak query, the wrong place, a barely-missed detail). borromeanRings runs the loop.
...
def research_until_saturated(
    ...
    **saturates** — ``dry_rounds`` consecutive rounds that add no new source. Always
    ...
    gap-finder is the agent/LLM; borromeanRings runs the loop and decides saturation.
    """
    seen: set[str] = set()
    sources: list[Source] = []
    trail: list[str] = []
...
def synthesize(query: str, findings: Sequence[tuple[str, Verdict]]) -> CitedReport:
    ...
    prose may be elaborated by the agent later; what borromeanRings guarantees is that no
...
def enhanced_research(
    ...
    *,
    ...
    The agent's search (``agent_search_fn``) is the base; borromeanRings *enhances* it:
    ...
    or bare LLM — borromeanRings enhances, the agent performs.
    """
