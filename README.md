...
(`06_git_identity` exists but is intentionally excluded so external contributors pass
CI — see ADR-0019). The table above is the v0 core; the full set — what each enforces,
its config keys, and how to enable it — is catalogued in **`docs/CHECKS.md`**. The counts
below are generated by `./describe.sh --readme` and held to the registry by
`04_self_description` (ADR-0052):
