...
the rewriting (its own intelligence), and borromeanRings should only **enforce** that it happens, the
...
and **show the rewrite before proceeding**. Toggle lives in `borromeanrings.toml`
(`[prompt_rewriting].enabled`). borromeanRings writes nothing into the prompt itself.
...
- **borromeanRings rewrites the prompt itself** — rejected: contradicts the user's clarification and the
  autonomy principle; borromeanRings enforces, the agent performs.
- **Always-on, not toggle-able** — rejected: the user should control it; config-driven on/off fits
  the spine.

## Consequences
- (+) The agent's own intelligence does the rewriting; borromeanRings guarantees the process + context.
