# ADR-0015 — Per-language check sets (borromeanRings adjusts the gate to the project)

**Status:** Accepted

## Context
borromeanRings shipped a Python-only check set (ADR-0004: prove one stack first). But borromeanRings must
...
`verify.sh` reads **`[project].language`** (default `python`) from the project's `borromeanrings.toml` and
...
## Consequences
- (+) borromeanRings runs the *right* checks per project — it adjusts to context (language) as the
