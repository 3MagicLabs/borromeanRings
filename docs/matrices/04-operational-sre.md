...
| O9 | SLOs are declared in the repo as data (indicator, objective, window), and every alert references an SLO | gap → #138 (no sub-issue yet) | archetype | SRE Book ch. 4 "Service Level Objectives"; SRE Workbook (2018) ch. 2 "Implementing SLOs", ch. 5 "Alerting on SLOs" |
| O10 | Error-budget consumption is recorded per window; a release while the budget is exhausted is refused by policy | gap → #138 | telemetry | SRE Book ch. 3 "Embracing Risk" (error budgets); SRE Workbook ch. 2 |
...
| O12 | Releases are canaried (a fraction of traffic on the new version with an automatic abort) | gap → #138 | telemetry | SRE Workbook ch. 16 "Canarying Releases"; SRE Book ch. 8 |
...
| O14 | Every incident produces a blameless postmortem document with action items tracked as issues | gap → #138 (process; presence of a `docs/postmortems/` entry linked from the incident) | telemetry | SRE Book ch. 15 "Postmortem Culture: Learning from Failure" |
| O15 | On-call playbooks exist for each alert (an alert without a playbook fails the alert-definition check) | gap → #138 | archetype | SRE Book ch. 11 "Being On-Call" (playbooks), ch. 14 "Managing Incidents" |
