...
| U4 | Form controls have an accessible label (`<label for>`, `aria-label`, or `aria-labelledby`) | gap → #138 (static, same parser as `15_a11y`; no sub-issue yet) | now | WCAG 2.2 SC 3.3.2 Labels or Instructions (A), SC 4.1.2 Name, Role, Value (A); axe-core rule `label` |
| U5 | Links have discernible text (no empty anchors, no bare "click here" by declared list) | gap → #138 (static) | now | WCAG 2.2 SC 2.4.4 Link Purpose (In Context) (A); axe-core rule `link-name` |
| U6 | Heading structure is well-formed: exactly one `<h1>` per document, no skipped levels | gap → #138 (static) | now | WCAG 2.2 SC 1.3.1 Info and Relationships (A); axe-core rules `page-has-heading-one`, `heading-order` |
| U7 | Text colour contrast meets the minimum ratio on the rendered page | gap → #138 (needs a rendered DOM — axe-core/pa11y on the heavy lane; explicitly deferred by ADR-0045) | telemetry (rendered) | WCAG 2.2 SC 1.4.3 Contrast (Minimum) (AA); axe-core rule `color-contrast` |
| U8 | All interactive elements are keyboard-reachable and focus is visible | gap → #138 (rendered) | telemetry (rendered) | WCAG 2.2 SC 2.1.1 Keyboard (A), SC 2.4.7 Focus Visible (AA), SC 2.4.11 Focus Not Obscured (Minimum) (AA, new in 2.2) |
| U9 | Interactive targets meet the minimum size or spacing | gap → #138 (rendered) | telemetry (rendered) | WCAG 2.2 SC 2.5.8 Target Size (Minimum) (AA, new in 2.2) |
