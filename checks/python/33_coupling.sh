...
extra="$(python3 -c "import json,sys; print(json.dumps({'worst_fan_out': int(sys.argv[1]), 'coupling_baseline': int(sys.argv[2]), 'worst_module': sys.argv[3]}))" "$current" "$baseline" "$worst" 2>/dev/null || echo '')"
